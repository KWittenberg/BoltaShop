...
namespace BoltaShop.Repository
{
    ...
    public async Task<Book> GetBookById(int id)
        {
            var bookDetails = await this.db.Books
                .Include(am => am.AuthorsBooks).ThenInclude(a => a.Author)
        ...
        public async Task<BookDropdownsViewModel> GetBookDropdownsValues()
        {
            var response = new BookDropdownsViewModel()
            {
                ...
                //Cinemas = await this.db.Cinemas.OrderBy(n => n.Name).ToListAsync(),
                //Producers = await this.db.Producers.OrderBy(n => n.FullName).ToListAsync()
            };
